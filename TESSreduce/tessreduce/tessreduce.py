...
def spacetime_lookup(ra,dec,time=None,buffer=0,print_table=True):
    ...
    if len(outSecs) > 0:
        ...
        for i in range(len(disc_start)):
            ...
            tr_list += [[ra, dec, secs.Sector.values[i], cover, outCam[i], outCcd[i]]]
        if print_table: 
...
def viewed_targets(infoFrame,tables=False,iterate=False):
    # -- Check each GRB to see if TESS may have covered it -- # 

    
    headers = [x.lower() for x in infoFrame.columns]
    broke = False

    try:
        raInd = headers.index('ra')
        ra = infoFrame[infoFrame.columns[raInd]]
    except ValueError:
        print('No right ascension column found. Please name frame columns "ra", "dec", "time".')
        broke = True
    try:
        decInd = headers.index('dec')
        dec = infoFrame[infoFrame.columns[decInd]]
    except ValueError:
        print('No declination column found. Please name frame columns "ra", "dec", "time".')
        broke = True
    try:
        timeInd = headers.index('time')
        time = infoFrame[infoFrame.columns[timeInd]]
    except ValueError:
        print('No time column found. Please name frame columns "ra", "dec", "time".')
        broke = True
    ...
    if not broke:
        ...
        for ii in range(len(infoFrame)):
            try: 
                if iterate:
                    print(ii)
                
                obj = spacetime_lookup(ra[ii],dec[ii],time[ii],print_table=tables)
        ...
        obsFrame = infoFrame.loc[observed]
